...
type SessionData = {
  ...
  whiteboardData?: any; // Added to type definition for sync
};
...
export default function LiveClassPage() {
  ...
  // 1. Firebase Listener: User Profile
  useEffect(() => {
  ...
  // 2. Firebase Listener: Class Details
  useEffect(() => {
    if (!firestore) return;
  ...
  // 3. Firebase Listener: Session Data (Syncs Recording & Whiteboard State)
  useEffect(() => {
    if (!firestore) return;
    const unsub = onSnapshot(doc(firestore, 'classes', classId, 'session', 'current'), (doc) => {
      if (doc.exists()) {
        ...
        // Auto-switch view based on whiteboard activity
    ...
    return () => unsub();
  }, [firestore, classId, viewMode]); // Added viewMode to dependency to prevent loops
  ...
  const handleLeaveSession = async () => {
    if (!user || !firestore) return;
    try {
      if (isTeacher && isRecording) await toggleRecording();
    ...
    if (isRecording) {
      ...
      try {
        ...
        stream.getTracks().forEach((track) => {
          track.onended = () => { if (isRecording) toggleRecording(); };
        });
        ...
        setIsRecording(true);
        toast({ title: 'Recording started!', description: `Format: ${fileExtension.toUpperCase()}` });
      } catch (err) {
        console.error('Failed to start recording:', err);
        toast({ variant: 'destructive', title: 'Failed to start recording' });
      }
    }
  };
  ...
  const jitsiUrl = `https://8x8.vc/vpaas-magic-cookie-7bb0b1ee8df54facb392382c0007102d/${jitsiRoom}#config.startWithVideoMuted=true&config.startWithAudioMuted=false`;
  ...
  // Handler for Whiteboard Sync (Sending data to Firebase)
  const handleWhiteboardSync = async (json: any) => {
    if (!isTeacher || !firestore) return;
    // In a real app, you might want to throttle this or save to a sub-collection 'whiteboard_events'
    // For now, we update the session doc directly (be careful of size limits)
    await updateDoc(doc(firestore, 'classes', classId, 'session', 'current'), {
        whiteboardData: json
    });
  };
    ...
    <>
      ...
      {showJoinDialog && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
          <div className="bg-white dark:bg-gray-900 p-8 rounded-xl shadow-2xl max-w-sm w-full mx-4">
            <h2 className="text-2xl font-bold mb-4">{isTeacher ? 'Start Live Class' : 'Join Class'}</h2>
      ...
      <div className="flex flex-col h-screen bg-gray-100">
        ...
        <header className="bg-white shadow-sm border-b px-4 py-3 flex items-center justify-between z-20">
          ...
          <div className="flex items-center gap-2">
            {sessionData?.whiteboardActive && (
              <Button size="sm" variant="outline" onClick={cycleViewMode} className="gap-2">
        ...
        <div className="flex-1 flex relative overflow-hidden">
          ...
          {/* Whiteboard Container - Fixed Integration */}
          <div
            className={`transition-all duration-300 bg-white relative ${
              viewMode === 'video-only' ? 'w-0 opacity-0' :
              viewMode === 'split-view' ? 'w-1/2 border-l-2' :
              'w-full'
            }`}
          >
             {/* Only render whiteboard if view mode permits to save resources */}
             {viewMode !== 'video-only' && (
                 <EnhancedCanvasEditor 
                    className="w-full h-full"
                    readOnly={!isTeacher} // Students are read-only
                    onSync={handleWhiteboardSync} // Teacher sends updates
                    syncData={sessionData?.whiteboardData} // Students receive updates
                 />
             )}
          </div>
