...
import { Power, PhoneOff, Monitor, MonitorOff, Mic, MicOff, Circle } from 'lucide-react';
import { useToast } from '@/hooks/use-toast';
import { Whiteboard } from '@/components/whiteboard/Whiteboard'; // Adjust path as needed
...
export default function LiveClassPage() {
  ...
  const handleJoin = async (asObserver = false) => {
    ...
    try {
      setError(''); setShowJoinDialog(false);
  ...
  // RECORDING CONTROL (8x8 VPAAS supports this via JWT or direct API)
  const toggleRecording = async () => {
    if (!isTeacher || !iframeRef.current?.contentWindow) return;

    const api = (iframeRef.current.contentWindow as any).JitsiMeetExternalAPI;
    if (!api) {
    ...
    try {
      ...
      toast({ variant: 'destructive', title: 'Recording failed', description: 'Make sure you have recording enabled in your 8x8 dashboard' });
    }
  };

  const isLoading = userLoading || !profile || !classDetails || (!isInitialized && !showJoinDialog);

  // Enhanced Jitsi URL with recording capability
    ...
    <>
      ...
      <div className="flex flex-col h-screen bg-gray-100">
        ...
        <div className="flex-1 flex relative overflow-hidden">
          {/* Jitsi Video */}
          <div className={`absolute inset-0 transition-all duration-300 ${showWhiteboard ? 'w-1/2' : 'w-full'}`}>
          ...
          {/* Whiteboard Panel (Slide-in) */}
          <div className={`absolute right-0 top-0 h-full bg-white shadow-2xl transition-transform duration-500 ${showWhiteboard ? 'translate-x-0 w-1/2' : 'translate-x-full w-1/2'}`}>
            <div className="h-full flex flex-col">
              <div className="bg-gray-800 text-white p-3 flex justify-between items-center">
                <h3 className="font-bold">Whiteboard</h3>
                <Button size="sm" variant="ghost" onClick={() => setShowWhiteboard(false)}>✕</Button>
              </div>
